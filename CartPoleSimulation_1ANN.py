...
from numpy import sin, cos, deg2rad, rad2deg, tanh, transpose, matrix, r_, sign
...
class ANNController(Controller):
	...
	def inputNormalize(self, state):
		self.temp_state[0,0] = (state[0] - self.reference_p_ang) / self.res_angle
		self.temp_state[0,1] = (state[1] - self.reference_c_pos) / self.res_deviation
		self.temp_state[0,2] = (state[2] - self.reference_p_vel)
		self.temp_state[0,3] = (state[3] - self.reference_c_vel) / 5
	...
	def update(self, state):
		...
		#self.ANN()
		self.reflex(state)
		self.control_output[0] = self.reference_output + self.Z3 * self.gain + self.reflex_sig * self.reflex_gain 
...
class ANNController_balance(ANNController):
	def __init__(self):
		...
		#constants for the out of bounds function that is used to reset the simulation
		#and for the reward function
		self.res_angle = self.rew_angle + deg2rad(5)
		self.res_deviation = self.rew_deviation + 0.2
...
if __name__ == '__main__':
	...
	print "pole weight"
	print sim.controllers_arr[1].weights1[0,0]
	print "cart weight"
	print sim.controllers_arr[1].weights1[0,1]
	print "pole speed weight"
	print sim.controllers_arr[1].weights1[0,2]
	print "cart speed weight"
	print sim.controllers_arr[1].weights1[0,3]
	
	print "pole weight"
	print sim.controllers_arr[2].weights1[0,0]
	print "cart weight"
	print sim.controllers_arr[2].weights1[0,1]
	print "pole speed weight"
	print sim.controllers_arr[2].weights1[0,2]
	print "cart speed weight"
	print sim.controllers_arr[2].weights1[0,3]
